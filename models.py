...
def proxy2list(result_proxy):
    ...
    BEWARE: result_proxy will close after this (fetchall)
    """
...
def list2nested(data_list, new_list=None, lft=0, rgt=0):
    ...
    while lft <= d['lft'] < rgt:
        ...
        if d['rgt'] - d['lft'] == 1:
            ...
            new_list.append(d)
            print(d['item_name'],'<append')        
            d = data_list.pop(0)
        else:
            
            d['children'] = list2nested(data_list,new_list,d['lft'],d['rgt'])
            new_list.append(d)
            rint(d['item_name'],'<<child_append')
            d = data_list.pop(0)
...
class VegetablesConceptionModel(object):
    """querys to display conceptions of Veggi growing
    """
    def __init__(self):
        ...
        ## with mutch formating suff like concat and ad strings
        ## text( ) 

        #:selecting seed events
        self.plan_seed_sql= " select id, hint, square_of_field, area_name,Concat(coalesce(cast(amount_of_grow_kind_units as text),' '),' ', coalesce(growkind_name,' ') )AS growkind, length_of_field, plants_per_square, Concat (coalesce(cast(amount_of_plants as text),' '),'/',coalesce(cast(amount_of_seed as text),' ')) AS count_seed, cast('S' as character) AS action, week_of_plan_seed AS week, name,  Concat(coalesce(diversity_name,''),' ',coalesce(diversity_ident,'')) AS diversity, note, implementation_seeding_date AS imp_date, implementation_id AS imp_id FROM "+self._viewname_+" WHERE week_of_plan_seed is not NULL AND fraction_id IN (select * from ext_fraction( :fraction)) AND year_of_growth = :year "
        
        #:selecting implant events
        self.plan_plant_sql=" select id, hint, square_of_field, area_name,Concat(coalesce(cast(amount_of_grow_kind_units as text),' '),' ', coalesce(growkind_name,' '),' | ',coalesce(cast(planting_interval as text),'')) AS growkind, length_of_field, plants_per_square, Concat (coalesce(cast(amount_of_plants as text),' '),'/',coalesce(cast(amount_of_seed as text),' ')) AS count_seed, cast('P' as character) AS action, week_of_plan_plant AS week, name,  Concat(coalesce(diversity_name,''),' ',coalesce(diversity_ident,'')) AS diversity , note, implementation_planting_date AS imp_date, implementation_id AS imp_id FROM "+self._viewname_+" WHERE week_of_plan_plant is Not Null AND fraction_id IN (select * from ext_fraction( :fraction)) AND year_of_growth = :year "

        #:selecting harvest events
        self.plan_crop_sql= " select id, hint, square_of_field, area_name,Concat(coalesce(cast(amount_of_grow_kind_units as text),' '),' ', coalesce(growkind_name,' ')) AS growkind, length_of_field, plants_per_square, Concat (coalesce(cast(amount_of_plants as text),' '),'/',coalesce(cast(amount_of_seed as text),' ')) AS count_seed, cast('E' as character) AS action, week_of_plan_crop AS week, name,  Concat(coalesce(diversity_name,''),' ',coalesce(diversity_ident,'')) AS diversity , ' ', implementation_yilding_date AS imp_date, implementation_id AS imp_id  FROM "+self._viewname_+" WHERE week_of_plan_crop is not null AND fraction_id IN (select * from ext_fraction( :fraction)) AND year_of_growth = :year "
...
# "in Django Fraction is allready taken by python
# "and I have a autoinkrement at all tables"
#-------------------------------------------------------------------#
class FractionModel(object):
    ...
    attributes: fraction_id: integer primary key
                name: string
                product_tree_id: DB intern datetime type
                area_tree_id: DB intern datetime type
...
class TransactionModel(object):
    """handels product movments between fractions
    table acquisition
    table retraction
    ...
    def __init__(self, fraction_id, source_fraction_id="NULL"):
...
##
    ...
    def acqusitions_get_last_inserts(self,limit=12):
    ...
    def acqusitions_get_last_dates(self,limit=48):
